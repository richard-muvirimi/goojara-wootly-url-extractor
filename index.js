  ...
  .action(async (target, options) => {
    log4js.configure({
      ...
      categories: {
        default: {
          appenders: ["console"],
          level: options.verboseLevel || "info",
        },
      },
    });
    ...
    try {
      ...
      await page.goto(target, {
        timeout: Duration.fromObject({ minutes: 5 }).toMillis(),
      });
      ...
      let handle = await page.waitForSelector("#container iframe", {
        timeout: Duration.fromObject({ minutes: 5 }).toMillis(),
      });

      const iframe = await handle.contentFrame();

      const btn = await iframe.waitForSelector(".vid-holder .playh", {
        timeout: Duration.fromObject({ minutes: 5 }).toMillis(),
      });
      ...
      await iframe.waitForNavigation({
        timeout: Duration.fromObject({ minutes: 10 }).toMillis(),
      });
      ...
      logger.info(
        "Wootly Title: " + path.basename(title2, path.extname(title2)),
      );

      await iframe.waitForSelector("#dld a[href^='https://']", {
        timeout: Duration.fromObject({ minutes: 10 }).toMillis(),
      });

      logger.debug("waiting for download button");

      downloadUrl = await iframe.evaluate(
        "document.querySelector(\"#dld a[href^='https://']\").href",
      );
    } catch (e) {
      logger.error(e);
    } finally {
